/*
    ...
    3. Respond to each request with a single HTML
    ...
    *HINT2: Look up on the Deno Documention on how to
            read a file + permissions needed for the
            deno run command
*/

async function sampleHtmlHandler(_request: Request) {
  const file = await Deno.open("sample.html");
  const headers = { "Content-Type": "text/html" };
  return new Response(file.readable, { headers });
}

Deno.serve({ port: 9000 }, sampleHtmlHandler);
